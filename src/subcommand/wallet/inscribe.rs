...
println!("  Signed PSBT: {}", base64::encode(&serde_json::to_string(&signed_psbt).unwrap()));
...
file.write_all( base64::encode(&serde_json::to_string(&signed_psbt ).unwrap()).as_bytes() )?;
